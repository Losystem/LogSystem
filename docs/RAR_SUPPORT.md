...
1. **MIME type** : `application/vnd.rar`, `application/x-rar-compressed`
2. **Extension** : `.rar` seulement
3. **Taille maximale** : Par défaut 1 GB (configurable)
4. **Taille décompressée** : Limite contre les bombes ZIP
5. **Traversée répertoires** : Chemins `../` rejetés
6. **Fichiers sensibles** : `.exe`, `.dll`, `.so` filtrés
