...
router.get('/', async (req, res) => {
  try {
    const userScopeFilter = userScope(req);
    const { limit = 50, sort = 'timestamp', order = 'desc', last_id, after_id,
    ...
    // after_id: delta polling — only logs newer than this ID
    if (after_id) {
      sql += ' AND id > ?';
...
router.get('/analysis/:fingerprint', async (req, res) => {
  try {
    ...
    try {
      ...
      meta = metaRows[0] || {};
    } catch (_) { /* error_groups table may not exist yet */ }
