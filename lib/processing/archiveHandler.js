...
async function getUnrarExtractor() {
  ...
  try {
    ...
    if (wasmBinary) {
      logger.info({ event: 'unrar_wasm_loaded', path: loadedPath, isVercel: IS_VERCEL }, '[ARCHIVE]');
...
async function extractRar(buffer, filename, maxSize) {
  ...
  if (IS_RENDER) {
    throw new ArchiveError('RAR_UNSUPPORTED', 400,
      'Extraction RAR non disponible sur Render. Utilisez .zip, .7z, .gz ou .tar.gz à la place.');
  }
